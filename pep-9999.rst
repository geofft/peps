...
distribution
    ...
    that collects and publish the software and makes any
...
1. A standard unpatched CPython, without any special
   ...
   * Can it delete externally-installed packages? Because
     this is an unpatched, locally installed CPython not
     shpped through a distribution, there isn't really a
     concept of "externally-installed" packages in this use
     case; none of the ``sysconfig`` schemes of this
     interpreter should overlap any distribution-installed
     Python you may have on the same system. Still,
     technically, if you had some non-Python-specific
     mechanism for installing packages into the directories
     used by this interpreter, currently yes, ``pip
     install`` would be willing to delete them, and after
     this PEP also yes.
...
7. A build of Python as part of a specific type of
   distribution where direct users of Python or other
   languages in the distribution are expected or encouraged
   to make changes to the distribution itself if they wish
   to add libraries. Common examples include private
   "monorepos" at software development companies, where the
   users are software engineers and a single repository
   builds both third-party and in-house software.
   User-level package managers like Nixpkgs_ may also count,
   because they encourage users of Nix who are Python
   developers to `package their software for Nix`__.  In
   these cases, the distribution may want to provide
   guidance encouraging use of the distribution's own
   facilities for adding new package, along with a link to
   documentation. They may also have custom instructions for
   how to properly set up a virtual environment.
...
8. A distribution Python, used in the specific context of
   ...
   package - e.g., building a ``python-xyz`` RPM by using
...
9. A distribution Python used with ``PYTHONHOME`` to set up
   ...
   is just like a distribution Python (case 2):
   ...
   * Can it delete externally-installed packages? Currently
   ...
   unmodified upstream Python (case 1) and does not change:

   * Can you ``pip install`` without an override? Currently
     yes; after this PEP yes, because there is no marker
     file.

   * Can it delete externally-installed packages (assuming
     there is some well-defined concept of
     "externally-installed packages" in this context)?
     Currently yes; after this PEP yes, because the
     distribution's changes to ``sysconfig`` won't be
     visible, and there will only be one shared scheme.
...
boundary; it is to discourage well-intentioned changes from
