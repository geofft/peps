...
several representative use cases / contexts. Specfically, we
...
9    ``PYTHONHOME`` copied from        Currently yes; becomes no   Curretly yes; becomes no
...
1. A standard unpatched CPython, without any special
   ...
   any OS-installe packages are not visible or relevant
   here.
...
4. A distribution Python when used inside a virtual
   ...
   the answer remains no, but the reasoning becmoes more
...
5. A distribution Python when used in a single-application
   ...
   distribution-installed Python interpereters, even in
...
8. When building distribution Python packages for a
   ...
   packae build process. (Consider, for instance, building a
