...
new version of a package systemwide (e.g., with ``sudo pip
...
9. A distribution Python used with ``PYTHONHOME`` to set up
   ...
   * Can it delete externally-installed pavkages? Currently
...
If the contents of the ``EXTERNALLY-MANAGED`` file parses as
an ``email.message.EmailMessage`` using the routines in
``email.parser``, the installer should output an error
message from that file as part of its error. If
...
If the file does not parse as an ``EmailMessage``, then the
...
::
    ...
    To install Python packages systemwide, try apt install
...
* Consider packaging pipx_, a tool for installing
  ...
  Python when used directly (e.g., ``python3`` on Fedora or
  ``python3-full`` on Debian) depends on pipx in addition to
  pip.
...
* Distributions should place two separate paths on the
  ...
  ``/usr/local/bin/pip`` for the locally-built interpreter
  don't interfere with packages installed via
  ``/usr/bin/pip`` for the distribution interpreter.
...
the stnadard library supportfor virtual environments,
...
unexpectedly breaking a user's envoronment. That is to say,
...
for container images, pacage build environments, etc. to
...
executable, but that code needs to be supplied whe the
...
``sys.path`` and ignore the local system administator's
...
they use to implement the two-systemwide-directories
...
the standard libary (``sys`` and ``sysconfig``,
