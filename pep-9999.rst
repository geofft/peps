...
Put the file in ``sys.path`` - wouldn't have the desired behaviour for
``--user`` installs.
...
Allow distro software to run without the site-installed packages (a
"system python" vs "user python") - this is a good idea, but outside the
scope of this PEP
